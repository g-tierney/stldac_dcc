...
senTweets116_2020.dfm <- dfm(senTweets116_2020$text_clean,tolower = T,
                             remove = c(stopwords(source = "smart"),"hyperlink","amp"),
                             remove_punct = T)
...
#extract test set
set.seed(196)
test <- senTweets116_2020.dfm_trimmed@docvars %>% 
  group_by(screen_name) %>% 
  slice_sample(prop = 0.1) %>% 
  {senTweets116_2020.dfm_trimmed@docvars$docname_ %in% .$docname_} %>% 
  which
