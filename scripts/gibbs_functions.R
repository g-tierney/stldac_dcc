...
collapsed_gibbs_1topic_clusters <- function(alpha=1,eta=1,nu=1,
  ...
  for(iter in 1:(niter+nburnin)){
    ...
    for(d in sample(iter_docs,size = length(iter_docs),replace = F)){
      ...
      #remove from current counts
      ut[u,t] <- ut[u,t] - 1
      if(FALSE){ #is.dfm(dw)){ #big dw matrices are passed as dfm objects
      ...
      # w_to_t <- sapply(1:nT,function(t){
      #   denom <- prod((sum(tw[t,]) + nW*eta):(sum(tw[t,]) + nW*eta + sum(dw[d,])-1))
      #   num <- numerator_product_vec(dw[d,],tw[t,] + eta) %>% unlist %>% prod
      # 
      #   log(num) - log(denom)
      # })
      probs <- (u_to_t + w_to_t) %>% {. - max(.)} %>% exp %>% {./sum(.)}
